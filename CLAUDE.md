...
- Server → client: `balance`, `profile` ({ avatar }), `roulette:state`, `roulette:bets`, `roulette:outcome`, `bj:state`. State events carry the full public snapshot (not diffs), including `endsIn` and `duration` (ms of the current timed phase) for countdowns and progress bars.
