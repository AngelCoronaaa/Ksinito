...
- `db.js` — schema (`users`, `ledger`, `roulette_spins`, `avatars`, `chat_messages`, `transfers`; the old `sessions` table is dropped). There is no migration system: new columns are added idempotently at startup (see `users.public_id`, the random 8-digit player ID, backfilled for old rows; use `withPublicId()` when inserting users). It also detects whether `DATA_DIR` is on a container volume (`STORAGE`: `persistent`/`ephemeral`/`local`, exposed at `GET /api/health`), and uses `/data` automatically when a volume is mounted there.
...
### Socket protocol
- Client → server (with ack callback): `roulette:bet {type, value, amount}`, `roulette:clear`, `chat:send {channel, text}`, `bj:watch {table}` (switch the watched table; the socket joins only that table's room), `bj:sit {table, seat}`, `bj:leave`, `bj:bet {amount}`, `bj:action {action: hit|stand|double|split}`.
- Server → client: `balance`, `profile` ({ avatar }), `roulette:state`, `roulette:bets`, `roulette:outcome`, `bj:state` (includes the table `id`/`name`; the client ignores states from tables it isn't watching), `bj:lobby` (per-table `{id, name, seats, occupants, phase}`), `bj:outcome` (private), `chat:history`, `chat:message`, `transfer:received`. State events carry the full public snapshot (not diffs), including `endsIn` and `duration` (ms of the current timed phase) for countdowns and progress bars.
